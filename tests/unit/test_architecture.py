...
The layering is deliberately boring:
    ...
    acp.common   <- shared library, depends on nothing of ours
    acp.sim      <- may use common
    acp.ml       <- may use common
    acp.services <- may use common, sim, ml; never another service

Services communicate over Kafka and HTTP only. The shared package is a library
in the same sense a shared JAR is: it carries the wire contracts and utilities,
not behaviour that couples two services together.
"""
...
def test_shared_library_does_not_depend_on_its_consumers(path: Path) -> None:
    ...
    forbidden = {"acp.services", "acp.sim", "acp.ml"}
    offenders = {
        imported
